...
// Double_t phi, R5Probe* R5Detector::PrepareKalmanTrack(Double_t pt, Double_t
// lambda, Double_t mass, int charge, Double_t phi, Double_t x,Double_t y,
...
R5Probe::R5Probe()
    : fHits(0), fFakes(0), fNHitsFake(0), fInnerChecked(-1), fOuterChecked(-1),
      fTOFsignal(0) {}
...
R5Probe::R5Probe(R5Probe &src)
    : AliKalmanTrack(src), fHits(src.fHits), fFakes(src.fFakes),
      fNHitsFake(src.fNHitsFake), fInnerChecked(src.fInnerChecked),
      fOuterChecked(src.fOuterChecked), fTOFsignal(fTOFsignal) {}
...
void R5Probe::Print(Option_t *option) const {
  printf("M=%.3f Chi2=%7.2f (Norm:%6.2f) Checked: %d-%d, Hits: Total:%d "
         "Fakes:%d | Y:%+8.4f Z: %+8.4f LInt: %+.2f TOF: %+e|",
         GetMass(), GetChi2(), GetNormChi2(kTRUE), fInnerChecked, fOuterChecked,
         GetNumberOfClusters(), fNHitsFake, GetY(), GetZ(),
         GetIntegratedLength(), fTOFsignal);
  for (int i = 0; i < fgNLayers; i++) {
    if (!IsHit(i))
      printf(".");
    else
      printf("%c", IsHitFake(i) ? '-' : '+');
  }
  printf("|%s\n", IsKilled() ? " KILLED" : "");
  TString opt = option;
  if (!opt.IsNull())
...
//__________________________________________________________________________
Bool_t R5Probe::GetXatLabR(Double_t r, Double_t &x, Double_t bz,
                           Int_t dir) const {
  // Get local X of the track position estimated at the radius lab radius r.
  // The track curvature is accounted exactly
  //
  // The flag "dir" can be used to remove the ambiguity of which intersection to
  // take (out of 2 possible) 0  - take the intersection closest to the current
  // track position >0 - go along the track (increasing fX) <0 - go backward
  // (decreasing fX)
  //
  // special case of R=0
  if (r < kAlmost0) {
  ...
  if (TMath::Abs(crv) <= kAlmost0) {  // this is a straight track
    if (TMath::Abs(sn) >= kAlmost1) { // || to Y axis
      ...
      if (det < 0)
        return kFALSE; // does not reach raduis r
      x = fx;
      if (dir == 0)
        return kTRUE;
      det = TMath::Sqrt(det);
      ...
      if (!dir) {
        x = fx > 0 ? det
                   : -det; // choose the solution requiring the smalest step
        return kTRUE;
...
Bool_t R5Probe::PropagateToR(Double_t r, Double_t b, int dir, Bool_t tof) {
  ...
  while (1) {
    ...
    if (tof) {
      GetXYZ(xyz1);
      double dx = xyz1[0] - xyz0[0], dy = xyz1[1] - xyz0[1],
             dz = xyz1[2] - xyz0[2];
      AddTimeStep(sgn * TMath::Sqrt(dx * dx + dy * dy + dz * dz));
    }
    rcurr2 = xR * xR + GetY() * GetY();
    if (TMath::Abs(rcurr2 - rr) < kTiny || rr < kAlmost0)
      return kTRUE;
    //
...
Bool_t R5Probe::CorrectForMeanMaterial(const R5Layer *lr, Bool_t inward) {
  ...
  if (AliExternalTrackParam::CorrectForMeanMaterial(
          lr->fx2X0, inward ? lr->fXRho : -lr->fXRho, GetMass(), kTRUE)) {
    //  printf("after  at r=%.1f p=%.4f\n",lr->fR, P());
    return kTRUE;
  }
  AliDebug(2,
           Form("Failed to apply material correction, X/X0=%.4f", lr->fx2X0));
...
R5Cluster::R5Cluster(R5Cluster &src)
    : TObject(src), fY(src.fY), fZ(src.fZ), fX(src.fX), fPhi(src.fPhi),
      fID(src.fID) {}
...
R5Layer::R5Layer(char *name)
    : TNamed(name, name), fR(0), fZMax(0), fx2X0(0), fXRho(0), fPhiRes(0),
      fZRes(0), fEff(0), fIsDead(kFALSE), fActiveID(-1), fClMC() {
  Reset();
}
  ...
  if (fExtInward[kSigY2] < 0) { // inward is not defined
    if (fExtOutward[kSigY2] > 0) {
      for (int i = kNPointParam; i--;)
        fExtComb[i] = fExtOutward[i];
    } else {
      for (int i = kNPointParam; i--;)
        fExtComb[i] = -1; // not defined
      return;
    }
  } else {
    if (fExtOutward[kSigY2] > 0) { // both are defined, combine them
      Double_t detInw = 1. / (fExtInward[kSigY2] * fExtInward[kSigZ2] -
                              fExtInward[kSigZY] * fExtInward[kSigZY]);
      Double_t wyyInw = fExtInward[kSigZ2] * detInw,
               wzzInw = fExtInward[kSigY2] * detInw,
               wzyInw = -fExtInward[kSigZY] * detInw;
      Double_t detOut = 1. / (fExtOutward[kSigY2] * fExtOutward[kSigZ2] -
                              fExtOutward[kSigZY] * fExtOutward[kSigZY]);
      Double_t wyyOut = fExtOutward[kSigZ2] * detOut,
               wzzOut = fExtOutward[kSigY2] * detOut,
               wzyOut = -fExtOutward[kSigZY] * detOut;

      Double_t wyyCmb = wyyInw + wyyOut, wzzCmb = wzzInw + wzzOut,
               wzyCmb = wzyInw + wzyOut;
      Double_t detCmb = 1. / (wyyCmb * wzzCmb - wzyCmb * wzyCmb);

      Double_t wy = fExtInward[kPosY] * wyyInw + fExtInward[kPosZ] * wzyInw +
                    fExtOutward[kPosY] * wyyOut + fExtOutward[kPosZ] * wzyOut;
      Double_t wz = fExtInward[kPosZ] * wzyInw + fExtInward[kPosZ] * wzzInw +
                    fExtOutward[kPosY] * wzyOut + fExtOutward[kPosZ] * wzzOut;
...
void R5Layer::Print(Option_t *opt) const {
  TString opts = opt;
  opts.ToLower();
  printf("Lr%3d(A%3d) %10s R=%5.1f Zmx=%5.1f X2X0=%.3f XRho=%.3f SigY=%+.5f "
         "SigZ=%+.5f Eff:%+4.2f\n",
         GetUniqueID(), fActiveID, GetName(), fR, fZMax, fx2X0, fXRho, fPhiRes,
         fZRes, fEff);
  if (opts.Contains("t")) { // print tracking info
    printf("ExtInw: Y: %+.3e Z: %+.3e | %+.4e %+.4e %+.4e\n", fExtInward[kPosY],
           fExtInward[kPosZ], fExtInward[kSigY2], fExtInward[kSigZY],
           fExtInward[kSigZ2]);
    printf("ExtOut: Y: %+.3e Z: %+.3e | %+.4e %+.4e %+.4e\n",
           fExtOutward[kPosY], fExtOutward[kPosZ], fExtOutward[kSigY2],
           fExtOutward[kSigZY], fExtOutward[kSigZ2]);
    printf("ExtCmb: Y: %+.3e Z: %+.3e | %+.4e %+.4e %+.4e | Diag: %.3e %.3e "
           "%+.3e\n",
           fExtComb[kPosY], fExtComb[kPosZ], fExtComb[kSigY2], fExtComb[kSigZY],
           fExtComb[kSigZ2], fDiagErr[kDiagErr0], fDiagErr[kDiagErr1],
           fDiagErr[kDiagTheta]);
  }
  if (opts.Contains("c")) {
    printf("Cluster: Id: %+3d: %+7.4f:%+7.4f\n", fClMC.GetID(), fClMC.GetY(),
           fClMC.GetZ());
  }
}

//_________________________________________________________
void R5Layer::Diagonalize2x2Matrix(Double_t sigAA, Double_t sigBA,
                                   Double_t sigBB, Double_t &sig11,
                                   Double_t &sig22, Double_t &theta) {
...
R5Detector::R5Detector()
    : TNamed("test_detector", "detector"), fNLayers(0), fNActiveLayers(0),
      fFirstActiveLayer(-1), fLastActiveLayer(-1), fFirstActiveLayerTracked(-1),
      fLastActiveLayerTracked(-1), fLayers(), fBFieldG(5.),
      fIntegrationTime(0.02), // in ms
      fdNdEtaCent(2000),      // Multiplicity
      fDensFactorEta(1), fMaxChi2Cl(30.), fMaxNormChi2NDF(7.), fMinHits(4),
      fMaxSnp(0.8), fPropagateToOrigin(kFALSE), fTOFResolutionPS(30.),
      fProbeInMC0(), fProbeOutMC(), fProbeInward(), fESDtrack(),
      fNHitsAssigned(0) {
  //
  // default constructor
  //
...
R5Detector::R5Detector(char *name, char *title)
    : TNamed(name, title), fNLayers(0), fNActiveLayers(0),
      fFirstActiveLayer(-1), fLastActiveLayer(-1), fFirstActiveLayerTracked(-1),
      fLastActiveLayerTracked(-1), fLayers(), fBFieldG(5.),
      fIntegrationTime(0.02), // in ms
      fdNdEtaCent(2000),      // Multiplicity
...
void R5Detector::AddLayer(char *name, Double_t radius, Double_t zmax,
                          Double_t x2X0, Double_t xrho, Double_t phiRes,
                          Double_t zRes, Double_t eff) {
  //
  ...
  if (!newLayer) {
    ...
    TString lname = name;
    if (lname.Contains("vertex"))
      newLayer->SetBit(R5Layer::kBitVertex);
    //
    newLayer->fIsDead =
        (newLayer->fPhiRes < 0 && newLayer->fZRes < 0) || newLayer->fEff <= 0.;
    //
    if (fLayers.GetEntries() == 0)
      ...
      for (Int_t i = 0; i < fLayers.GetEntries(); i++) {
        ...
        if (radius > l->fR && (i + 1) == fLayers.GetEntries())
          fLayers.Add(newLayer); // even bigger then last one
      }
      //
    }
    //
...
void R5Detector::ClassifyLayers() {
  ...
  for (int il = 0; il < nl; il++) {
    ...
    if (!lr->IsDead()) {
      fLastActiveLayer = il;
      if (fFirstActiveLayer < 0)
        fFirstActiveLayer = il;
...
Int_t R5Detector::GetLayerID(int actID) const {
  ...
  if (actID < 0 || actID > fNActiveLayers)
    return -1;
  for (int i = fLastActiveLayer + 1; i--;) {
    if (GetLayer(i)->GetActiveID() == actID)
      return i;
  }
  return -1;
}
...
void R5Detector::Print(const Option_t *opt) const {
  ...
  for (Int_t i = 0; i < fLayers.GetEntries(); i++) {
    ...
    // printf("%d. %s \t %03.2f   \t%1.4f\t  ",i, tmp->GetName(),
    // tmp->GetRadius(), tmp->GetRadL() ); if (tmp->IsDead()) printf("  -  ");
...
Double_t R5Detector::ThetaMCS(Double_t mass, Double_t x2X0,
  ...
  // if ( RadLength > 0 ) theta  =  0.0136 * TMath::Sqrt(RadLength) / ( beta *
  // momentum );
  if (x2X0 > 0)
    theta = 0.0136 * TMath::Sqrt(x2X0) / (beta * momentum) *
            (1 + 0.038 * TMath::Log(x2X0));
  return (theta);
}

Double_t R5Detector::ProbGoodHit(Double_t radius, Double_t searchRadiusRPhi,
...
Double_t R5Detector::ProbGoodChiSqHit(Double_t radius,
...
Double_t R5Detector::ProbGoodChiSqPlusConfHit(Double_t radius, Double_t leff,
  ...
  Double_t c =
      -2 * TMath::Log(confLevel); // quantile at cut of confidence level
...
Double_t R5Detector::ProbNullChiSqPlusConfHit(Double_t radius, Double_t leff,
  ...
  Double_t alpha = (1 + 2 * TMath::Pi() * HitDensity(radius) *
                   ...
                   2;
  Double_t nullHit =
      (1 - leff + confLevel * leff) * TMath::Exp(-c * (alpha - 1. / 2));
  return (nullHit);
}
...
Double_t R5Detector::HitDensity(Double_t radius) {
  ...
  // et al. PRC 69, 054902 (2004) and PPT slides by Kai Schweda. Note that this
  // function assumes we are working in CM and CM**2 [not meters]. Based on work
  // by Yan Lu 12/20/2006, all radii and densities in centimeters or cm**2.

  //  Double_t MaxRadiusSlowDet = 0.1; //?   // Maximum radius for slow
  //  detectors.  Fast detectors
  if (radius < 0.01)
    return 0;
  //
  Double_t arealDensity =
      OneEventHitDensity(fdNdEtaCent, radius) + UpcHitDensity(radius);
  return (arealDensity);
}

Double_t R5Detector::OneEventHitDensity(Double_t multiplicity,
                                        Double_t radius) const {
  // This is for one event at the vertex.  No smearing.
  Double_t den = multiplicity / (2. * TMath::Pi() * radius * radius) *
                 fDensFactorEta; // 2 eta ?
...
void R5Detector::PrepareKalmanTrack(Double_t pt, Double_t eta, Double_t mass,
                                    int charge, Double_t phi, Double_t x,
                                    Double_t y, Double_t z, Double_t t) {
  ...
  probe.SetTOFsignal(t * 1e12 +
                     gRandom->Gaus(0, fTOFResolutionPS)); // TOF in ps
  ...
  trCov[R5Probe::kY2] = trCov[R5Probe::kZ2] = trCov[R5Probe::kSnp2] =
      trCov[R5Probe::kTgl2] = trCov[R5Probe::kPtI2] = 1e-20;
...
//________________________________________________________________________________
int R5Detector::TransportKalmanTrackWithMS(R5Probe *probTr,
                                           Bool_t applyMatCorr) {
  // Transport track till layer maxLr, applying random MS
  //
  const Double_t kcc = 2.99792458e-2;
  int nActLrOK = 0;
  Double_t r = TMath::Sqrt(probTr->GetX() * probTr->GetX() +
                           probTr->GetY() * probTr->GetY());
  fFirstActiveLayerTracked = -1;
  fLastActiveLayerTracked = -1;
  double xyz0[3], xyz1[3];
  probTr->GetXYZ(xyz0);
  for (Int_t j = 0; j < fNLayers; j++) {
    if (j > fLastActiveLayer)
      continue;
    R5Layer *lr = (R5Layer *)fLayers.At(j);
    if (lr->GetRadius() <= r)
      continue;
    if (!PropagateToLayer(probTr, lr, 1))
      break;

    {
      ...
      double q2pt = probTr->GetSigned1Pt(), tgl = probTr->GetTgl(),
             p2inv = q2pt * q2pt / (1 + tgl * tgl);
      Double_t correction = TMath::Sqrt(
          1. + probTr->GetMass() * probTr->GetMass() * p2inv); // 1/beta
      Double_t time = dst * correction / kcc;
      probTr->SetTOFsignal(time + probTr->GetTOFsignal());
    }

    if (TMath::Abs(probTr->GetSnp()) > fMaxSnp)
      break;
    Bool_t accZOK = lr->InZAcceptane(probTr->GetZ());
    if (lr->GetRadL() > 0 && applyMatCorr && accZOK) {
      ApplyMS(probTr, lr->GetRadL()); // apply MS
      if (!probTr->CorrectForMeanMaterial(lr, kFALSE))
        break;
    }
    //
    if (lr->IsDead())
      continue;
    if (fFirstActiveLayerTracked < 0)
      fFirstActiveLayerTracked = lr->GetActiveID();
    ...
    lr->GetMCCluster()->Set(probTr->GetY() + ry * lr->GetPhiRes(),
                            probTr->GetZ() + rz * lr->GetZRes(), probTr->GetX(),
                            probTr->GetAlpha(), -1);
...
Bool_t R5Detector::PropagateToLayer(R5Probe *trc, const R5Layer *lr,
                                    int dir) const {
  // bring the track to layer and rotat to frame normal to its surface
  if (!trc->PropagateToR(lr->fR, fBFieldG, dir))
    return kFALSE;
  //
...
Bool_t R5Detector::UpdateTrack(R5Probe *trc, R5Layer *lr, R5Cluster *cl) const {
  ...
  if (!trc->AliExternalTrackParam::Update(meas, measErr2)) {
    AliDebug(2, Form("layer %s: Failed to update the track by measurement "
                     "{%.3f,%3f} err {%.3e %.3e %.3e}",
                     lr->GetName(), meas[0], meas[1], measErr2[0], measErr2[1],
                     measErr2[2]));
    if (AliLog::GetGlobalDebugLevel() > 1)
      trc->Print("l");
    return kFALSE;
...
Bool_t R5Detector::ProcessTrack(const TParticle *part) {
  return ProcessTrack(
      part->Pt(), part->Eta(), part->GetMass(),
      TDatabasePDG::Instance()->GetParticle(part->GetPdgCode())->Charge() / 3,
      part->Phi(), part->Vx(), part->Vy(), part->Vz(), part->T());
}

Bool_t R5Detector::ProcessTrack(Double_t pt, Double_t eta, Double_t mass,
                                int charge, Double_t phi, Double_t x,
                                Double_t y, Double_t z, Double_t t) {
  ...
  if (fLastActiveLayerTracked < 0)
    return kFALSE; // no hits
  ...
  for (int ilr = outerTracked + 1; ilr--;) {
    ...
    if (!probeInw.PropagateToR(lr->GetRadius(), fBFieldG, kInward))
      break;
    if (TMath::Abs(probeInw.GetSnp()) > GetMaxSnp())
      break; // too large snp
    ...
    if (accZOK && !lr->IsDead()) {
      ...
      if (cl->IsValid()) {
        if (!probeInw.PropagateToCluster(cl, fBFieldG))
          return kFALSE; // track was not propagated to cluster frame
        ...
        if (!UpdateTrack(&probeInw, lr, cl))
          break;
      } else {
        lr->SetExtInward(&probeInw); // set extrapolation errors
      }
    }
    if (accZOK && !probeInw.CorrectForMeanMaterial(lr, kTRUE))
      return kFALSE;
    innerReached = ilr;
  ...
  Bool_t tofOK = probeInw.IsHit(
      fNActiveLayers -
      1); // TOF info is available for the tracks reaching last layer
  if (tofOK)
    tofOK &= r5Tmp.PropagateToDCA(&vtx0, fBFieldG, 999.);
  if (tofOK && (tofOK &= r5Tmp.RotateParamOnly(probeInw.GetAlpha())) &&
      r5Tmp.GetX() < probeInw.GetX()) {
  ...
  for (int ilr = innerReached; ilr < fNLayers; ilr++) {
    R5Layer *lr = GetLayer(ilr);
    if (!probeOut.PropagateToR(lr->GetRadius(), fBFieldG, kOutward, tofOK))
      break;
    if (TMath::Abs(probeOut.GetSnp()) > GetMaxSnp())
      break; // too large snp
    ...
    if (accZOK && !lr->IsDead()) {
      ...
      cl = lr->GetMCCluster();
      if (cl->IsValid()) {
        if (!probeOut.PropagateToCluster(cl, fBFieldG, tofOK))
          return kFALSE; // track was not propagated to cluster frame
        lr->SetExtOutward(&probeOut); // set extrapolation errors
        if (!UpdateTrack(&probeOut, lr, cl))
          break;
      } else {
        lr->SetExtOutward(&probeOut); // set extrapolation errors
      }
    }
    if (accZOK && !probeOut.CorrectForMeanMaterial(lr, kFALSE))
      return kFALSE;
    outerReached = ilr;
  ...
  //
  if (fProbeInward.GetNHits() < GetMinHits())
    return kFALSE;
  if (fPropagateToOrigin) {
    AliExternalTrackParam trTmp = fProbeInward;
    if (trTmp.PropagateToDCA(&vtx0, fBFieldG, 999.)) {
      double rDCA = TMath::Sqrt(trTmp.GetX() * trTmp.GetX() +
                                trTmp.GetY() * trTmp.GetY());
      if (!ExtrapolateToR(&fProbeInward, rDCA))
        return kFALSE;
      if (!fProbeInward.PropagateToDCA(&vtx0, fBFieldG, 999.))
        return kFALSE;
      //      double rCurr = TMath::Sqrt(fProbeInward.GetX()*fProbeInward.GetX()
      //      + fProbeInward.GetY()*fProbeInward.GetY());
    }
  }
...
Bool_t R5Detector::ExtrapolateToR(R5Probe *probe, Double_t r) const {
  ...
  Double_t curR2 =
      probe->GetX() * probe->GetX() + probe->GetY() * probe->GetY();
  int dir = 0;
  int lrId0 = -1,
      lrIdTgt = -1; // 1st layer to cross and last layer in given direction
  double r2tgt = r * r;
  double df2 = curR2 - r2tgt;
  if (TMath::Abs(df2) < 1e-4)
    return kTRUE;
  if (df2 < 0.) {
    dir = 1; // outward
    lrIdTgt = fNLayers;
  ...
  if (lrId0 != -1) {
    for (int ilr = lrId0; ilr != lrIdTgt; ilr += dir) {
      ...
      if (!PropagateToLayer(&probeC, lr, dir))
        return kFALSE; // failed to propagate
      if (!probeC.CorrectForMeanMaterial(lr, dir))
        return kFALSE;
    }
  }
...
Double_t R5Detector::Chi2ToCluster(const R5Layer *lr, const R5Probe *trc,
                                   const R5Cluster *cl) const {
  Double_t meas[2] = {cl->GetY(), cl->GetZ()}; // ideal cluster coordinate
  Double_t measErr2[3] = {lr->GetPhiRes() * lr->GetPhiRes(), 0,
                          lr->GetZRes() * lr->GetZRes()};
...
const AliESDtrack *R5Detector::GetProbeTrackInwardAsESDTrack() {
  ...
  if (fProbeInward.GetIntegratedLength() > 0) {
    fESDtrack.SetStatus(AliESDtrack::kTIME | AliESDtrack::kTOFout |
                        AliESDtrack::kTOFpid);
...
void R5Detector::AddESDTrackToEvent(AliESDEvent *esdEv,
                                    const AliESDtrack *trc) {

  int id = esdEv->AddTrack(trc);
  AliESDtrack *trnew = esdEv->GetTrack(id);
  // if needed account TOF info
  if (trc->IsOn(
          AliESDtrack::kTOFpid)) { // register TOF signal and time integrals
    ...
    AliESDTOFHit *ht = new ((*arrHt)[idTOF])
        AliESDTOFHit(tofSig, tofSig, 1., 0, lbl, 0, 0, idTOF, 0, 0, 0);
