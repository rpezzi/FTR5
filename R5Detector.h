...
public:
  ...
  R5Cluster(Double_t y = 0, Double_t z = 0, Double_t x = 0, Double_t phi = 0,
            Int_t id = -1)
      : fY(y), fZ(z), fX(x), fPhi(phi), fID(id) {}
  ...
  Double_t fPhi;
  Int_t fID;
  void Set(Double_t y, Double_t z, Double_t x, Double_t phi, int id) {
    fY = y;
    fZ = z;
    fX = x;
    fPhi = phi;
...
public:
  ...
  Bool_t IsHit(Int_t lr) const {
    return (lr < fgNLayers) ? IsWBit(fHits, lr) : kFALSE;
  }
  Bool_t IsHitFake(Int_t lr) const {
    return (lr < fgNLayers) ? IsWBit(fFakes, lr) : kFALSE;
  }
  ...
  static Bool_t IsWBit(const UInt_t &patt, const UInt_t bit) {
    return patt & (0x1 << bit);
  }
...
inline Bool_t R5Probe::PropagateToCluster(R5Cluster *cl, Double_t b, bool tof) {
  ...
  if ((TMath::Abs(cl->GetPhi() - GetAlpha()) > 1e-4 && !Rotate(cl->GetPhi())) ||
      (TMath::Abs(cl->GetX() - GetX()) > 1e-4 &&
       !AliExternalTrackParam::PropagateTo(cl->GetX(), b))) {
    AliDebugF(2, "Failed to propagate track to cluster at phi=%.3f X=%.3f",
              cl->GetPhi(), cl->GetX());
  ...
  if (tof) {
    GetXYZ(xyz1);
    double sgn = xyz0[0] * xyz0[0] + xyz0[1] * xyz0[1] <
...
public:
  ...
  static void Diagonalize2x2Matrix(Double_t sigAA, Double_t sigBA,
...
public:
  ...
  Bool_t ProcessTrack(Double_t pt, Double_t eta, Double_t mass, int charge,
                      Double_t phi, Double_t x = 0., Double_t y = 0.,
  ...
  void AddLayer(char *name, Double_t radius, Double_t zmax, Double_t radL,
                Double_t xrho = 0., Double_t phiRes = -1, Double_t zRes = -1,
                Double_t eff = -1);
  ...
  void SetIntegrationTime(Double_t integrationTime) {
  ...
  Double_t ProbGoodHit(Double_t radius, Double_t searchRadiusRPhi,
                       Double_t searchRadiusZ);
  Double_t ProbGoodChiSqHit(Double_t radius, Double_t searchRadiusRPhi,
                            Double_t searchRadiusZ);
  Double_t ProbGoodChiSqPlusConfHit(Double_t radius, Double_t leff,
                                    Double_t searchRadiusRPhi,
                                    Double_t searchRadiusZ, Double_t confL);
  Double_t ProbNullChiSqPlusConfHit(Double_t radius, Double_t leff,
                                    Double_t searchRadiusRPhi,
                                    Double_t searchRadiusZ, Double_t confL);
  ...
  Bool_t IsZero(Double_t val, Double_t tol = 1e-9) const {
  ...
  R5Layer *GetActiveLayer(Int_t actID) const {
    int pid = GetLayerID(actID);
    return pid < 0 ? 0 : GetLayer(pid);
  }
  R5Layer *GetLayer(const char *name) const {
    return (R5Layer *)fLayers.FindObject(name);
  }
  ...
  void SetMaxNormChi2NDF(Double_t cut = 5.) {
    fMaxNormChi2NDF = cut > 0 ? cut : 9;
  }
  ...
  void PrepareKalmanTrack(Double_t pt, Double_t eta, Double_t mass, int charge,
                          Double_t phi = 0, Double_t x = 0., Double_t y = 0.,
                          Double_t z = 0., Double_t t = 0.);
  ...
  Double_t Chi2ToCluster(const R5Layer *lr, const R5Probe *trc,
...
protected:
  ...
  Int_t fFirstActiveLayerTracked; // id of first active layer really used for
                                  // tracking of given pt
  Int_t fLastActiveLayerTracked;  // id of last active layer really used for
                                  // tracking of given pt
  ...
  AliESDtrack
      fESDtrack; // place-holder to transfer reconstructed track as ESDtrack
  Int_t fNHitsAssigned;

  //
  static Double_t
      fgVtxConstraint[2]; // if both positive, the vertex is used as constraint
                          // (accounted in chi2 but not in update)
